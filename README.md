...
- 使用原创水獭终端吉祥物与河流主题配色，品牌标识不依赖通用机器人图形。
- 支持 `Ctrl+O` 全屏会话记录：普通视图折叠冗长工具明细，全屏记录自动展开完整审计内容。
- 长会话支持 `PgUp / PgDn` 滚动；离开底部后停止自动跟随，`Esc` 先回到最新内容。
...
- `Ctrl+O`：打开或关闭全屏会话记录
- `PgUp / PgDn`：按屏滚动长会话
- `Esc`：执行中优先中止；浏览历史时先回到底部；全屏记录位于底部时退出
- 普通会话只展示工具摘要，完整输入、输出和耗时可在全屏记录中查看
- 审批或配置向导出现时会自动退出全屏记录，保证关键操作始终可见
